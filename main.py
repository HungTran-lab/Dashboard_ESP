...
def resource_path(relative_path: str) -> str:
    """
    Trả về đường dẫn thực tế của file resource (VD: dashboard.ui),
...
class PSWKitWindow(QMainWindow):
    def __init__(self):
        ...
        # ===== Biến trạng thái =====
        self.ser = None
        self.led_on = False          # LED on-board (SPARE2)
        ...
        # Tìm đúng QSlider theo objectName trong .ui
        ...
        for s in (self.sliderR, self.sliderG, self.sliderB):
            if isinstance(s, QSlider):
                ...
                # nếu .ui đang sai, log để biết tên nào lỗi
                if s is not None:
        ...
        # thay đổi giá trị -> update labels + preview
        ...
        self.update_rgb_labels()        # cập nhật số + ô preview màu
        self.update_conn_label(False)   # label CONNECTED/DISCONNECTED
        self.update_all_relay_labels()  # tất cả relay OFF lúc start

        # ===== Khởi tạo đồ thị ADC1 =====
        self.adc_history = []
        self.history_len = 200          # số điểm tối đa trên đồ thị
        self.init_plot()
    ...
    def update_conn_label(self, connected: bool):
        """Cập nhật màu + text của labelStatus khi connect/disconnect."""
        if connected:
            text = "CONNECTED"
            style = (
    ...
    def toggle_connect(self, checked):
        if checked:
            ...
            try:
                ...
                self.handshake_ok = False      # chuẩn bị chờ KIT=...
                self.send_cmd("INFO")
    ...
    def parse_line(self, line: str):
        ...
        # Thông tin board trả về sau INFO: KIT=PSW_ESP32;FW=...
        if line.startswith("KIT="):
            # Chỉ beep lần đầu sau khi connect (tránh người ta gõ INFO nhiều lần cũng kêu hoài)
    ...
    def update_rgb_labels(self):
        # nếu slider chưa tìm được hoặc không phải QSlider -> coi như 0
    ...
    def update_color_preview(self, r, g, b):
        # Chọn màu chữ trắng/đen tùy độ sáng
    ...
    # ------------------------------------------------------------------
    def load_ads(self):
        """Gửi lệnh ADS để đọc ADS1115 A0/A1 một lần."""
        self.send_cmd("ADS")
    ...
    def send_custom_cmd(self):
        """Đọc lệnh từ editCmd và gửi thẳng xuống ESP32."""
        ...
        self.send_cmd(cmd)
        # Optional: giữ lại text nhưng bôi đen, để gõ lệnh mới nhanh
        self.editCmd.selectAll()

    def set_sio(self, idx: int, state: int):
        """
        Điều khiển SIOx từ QCheckBox.
    ...
    def show_help(self):
        """Hiển thị danh sách API serial cho PSW ESP32 KIT."""
        text = (
            ...
            "Đọc trạng thái:\n"
            "  READ            → STATUS;ADC=a1,a2,a3;S=s1,s2,s3,s4,s5;\n"
